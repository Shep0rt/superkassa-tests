...
class KkmUserDeleteRegressionTest : BaseTest() {
    @Test
    ...
    fun shouldReturnOkFieldWithExpectedType() {
        val response = deletePreparedUser()
        val actualValue = response[OK_FIELD]

        SoftAssertions().apply {
            assertThat(response)
                .withFailMessage(ApiContractErrorMessages.documentedFieldMissing(ENDPOINT, OK_FIELD, RESPONSE_STRUCTURE))
    ...
    inner class AuthorizationRegressionTests {
        private var userToDeleteAfterTest: CreatedUser? = null

        @AfterEach
        fun `Очищаем тестовые данные после проверки авторизации`() {
            val createdUser = userToDeleteAfterTest ?: return
            userToDeleteAfterTest = null
        ...
        fun shouldReturnUnauthorizedWithoutAuthorization() {
            val createdUser = prepareUserForAuthorizationCheck()
        ...
        private fun prepareUserForAuthorizationCheck(): CreatedUser =
            prepareUserForDeletion().also { userToDeleteAfterTest = it }
    }
    ...
    private fun deletePreparedUser(): Map<String, Any?> {
        val createdUser = prepareUserForDeletion()
        return deleteUser(createdUser)
    }
